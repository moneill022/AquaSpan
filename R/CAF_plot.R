...
#' @export
#'
#' @examples Wild_CAF(WildDATA, species == "french angel", age, CAF)

Wild_CAF<- function(WildDATA, species, age, CAF ) {
  label <- rlang::englue("Cumulative Age Frequency for {{ species }}")
  ...
  WildDATA |>
    ...
    labs(x = "Age (Years)", y = "Relative Frequency (%)", title = label )+
